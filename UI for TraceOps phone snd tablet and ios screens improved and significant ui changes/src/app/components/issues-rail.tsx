...
export function IssuesRail({ issues, selectedIssueId, onSelectIssue, isMobile, isTablet, isWide }: IssuesRailProps) {
  const activeIssues = issues.filter(i => i.status === 'open' || i.status === 'investigating');
  const resolvedIssues = issues.filter(i => i.status === 'resolved');
  const hasSelection = selectedIssueId !== null;

  const width = isMobile ? 'auto' : isTablet ? '200px' : isWide ? '280px' : '252px';
